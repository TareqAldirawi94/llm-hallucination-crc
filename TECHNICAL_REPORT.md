...
The conformal layer works; the base detector is the bottleneck, and off-the-shelf
embeddings do not change that on this benchmark.
...
hallucination, and conditioning on domain exposes the true signal. The conformal layer is
sound; the base detector is the bottleneck, and the path forward is a better detector, not
